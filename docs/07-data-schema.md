...
`history`配列で日次蓄積する。`index_per`/`index_pbr`と`index_per_percentile_5y`等は、**T-17バックフィルで導入したトップレベル追加フィールド`valuation_history`から算出する**:

- `valuation_history.jp`: 日経平均プロフィル公式アーカイブの指数PER/PBR日次系列(加重平均ベース、5年分)。取得元はCloudflare保護でHTTP直叩き不可のため、スパイク成果物ファイル経由の一回限りバックフィル(docs/06-spikes/T-17-backfill.md)
- `valuation_history.us`: multpl.comのS&P500 PER(月次、trailing "as reported")・PBR(四半期)系列
- 各系列要素は`{date, value}`。パーセンタイルは「系列最新日から遡る5年ウィンドウ内の自己時系列パーセンタイル」(pipeline/metrics/factors.py `apply_valuation_history`)。系列の最新日を`index_per_as_of`/`index_pbr_as_of`で明示する
- **日米で定義・頻度が異なる**(日本=日次・前期基準利益、米国=月次/四半期・as reported)。水準の日米直接比較は不可で、各市場の自己時系列内の位置のみ有効。この注記は`source_note`と`valuation_history.*.definition`に格納済み
- 日次パイプライン(pipeline/daily.py)は`valuation_history`を保持し、毎回そこから最新値・パーセンタイルを再計算する(バックフィルが翌日の日次実行で消えない)

### 3.4 `factor_return_*` の注記(T-17バックフィル)

`factors/*.json` の `history[].factor_return_1m/3m/1y` は、T-17バックフィル(pipeline/backfill.py)では
**「現在の分位該当銘柄(等ウェイト)のトレーリングリターン − ユニバース等ウェイト平均」(%ポイント)の近似値**として算出している。
現在の構成銘柄で固定して過去リターンを遡るため、**生存バイアス・構成入れ替えの先読みバイアスを含む**
(例: モメンタムの1年値は選別変数そのものを測るため大きく正に出る)。
厳密なヒストリカル分位バックテストは**T-15**で実施する。
この注記文言は各`factors/*.json`のトップレベル`factor_return_note`フィールドに格納しており、サイト表示側はこれをそのまま表示できる。
日次パイプラインは同一日付の再実行時にバックフィル済みの`factor_return_*`と`factor_return_note`を保持する(null退行しない)。翌日以降の新規エントリはnullで作られる(T-15までは日次では算出しない)。
